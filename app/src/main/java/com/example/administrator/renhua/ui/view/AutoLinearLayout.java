...
public class AutoLinearLayout extends FrameLayout {
    ...
    int mOrientation;
    int mGravity = Gravity.TOP | GravityCompat.START;

    public final static int HORIZONTAL = 0;
    public final static int VERTICAL = 1;

    ArrayList<ViewPosition> mListPositions = new ArrayList<>();
     ...
     */
    void updateChildPositionVertical(int height, int totalSize, int column, int maxChildWidth) {
     ...
     */
    void updateChildPositionHorizontal(int width, int totalSize, int row, int maxChildHeight) {
        for (int i = 0; i < mListPositions.size(); i++) {
            ViewPosition pos = mListPositions.get(i);
            final View child = getChildAt(i);
    ...
    void updateLeftPositionByGravity(ViewPosition pos, int size, int gravity) {
    ...
    void updateTopPositionByGravity(ViewPosition pos, int size, int gravity) {
    ...
    void layout(View child, ViewPosition pos) {
        LayoutParams lp = (LayoutParams) child.getLayoutParams();
