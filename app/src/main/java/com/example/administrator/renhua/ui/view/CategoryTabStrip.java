...
public class CategoryTabStrip extends HorizontalScrollView {
    ...
    public CategoryTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        Log.d("reg", "screenWidth:" + screenWidth);
    }
    ...
    private void addTab(final int position, String title) {
        ...
        if (tabCount > 5) {
            //设置子tab的margin
            defaultTabLayoutParams.setMargins(0, 0, 40, 0);
        } else {
            if (position == 0){
                defaultTabLayoutParams.setMargins(((screenWidth - (category_text.getWidth() * tabCount)) / (tabCount * 4))*2 - 30, 0, ((screenWidth - (category_text.getWidth() * tabCount)) / (tabCount * 4)) - 4, 0);
            }else {
                defaultTabLayoutParams.setMargins(((screenWidth - (category_text.getWidth() * tabCount)) / (tabCount * 4)) - 4, 0, ((screenWidth - (category_text.getWidth() * tabCount)) / (tabCount * 4)) - 4, 0);
            }
        }
